...
1. **Discovery** (`discover.py`). A tool-using agent (web search, fetch, JSON inspection, hidden API detection in JS bundles, WordPress/Events-Calendar probes) finds the URL that lists the venue's upcoming events. It prefers JSON APIs over HTML and must verify the candidate contains dated events before answering.
2. **Schema generation** (`schema_gen.py`). A second agent writes a *declarative* extraction schema (CSS selectors or JSON paths, date formats, exclusions). Nothing generated is executed as code: the schema is data interpreted by `apply.py`. The schema is accepted only if, executed on the same page, it agrees (Jaccard >= 0.7 on title+date pairs) with an independent grounded LLM extraction of that page (`extract.py`). Disagreements are fed back to the generator; after three failed attempts the venue is flagged.
...
OpenRouter, defaults: `anthropic/claude-sonnet-5` for discovery/generation/reference extraction, `google/gemini-3.8-flash` as verifier. Roughly $0.30-0.60 per venue *once* (discovery + schema), then a few cents per new HTML event. Unchanged venues cost nothing.
