...
@pytest.mark.parametrize("key", ["millebabords", "cira"])
def test_non_listing_pages_return_none(key, today):
    # Mille Bâbords is a flat SPIP page (no repeated card); CIRA lists two causeries in prose.
    assert induce_schema(_doc(key), today) is None
